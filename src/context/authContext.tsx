...
export const AuthProvider = ({ children }: ChildrenProps) => {
  ...
  const signWithGoogle = async (access_token: string) => {
    try {
      setStatus(authStatus.loading)
      const userResponse = await authLoginGoogle({ access_token })
      dispatch(createUser(userResponse))
      setStatus(authStatus.authenticated)
    } catch (error: Error | any) {
      if (!error.errorInfo) {
  ...
  const value = useMemo(() => {
    return { status, error, signOut, signWithEmailPassword, isMutating: isLoggingIn ?? isLoggingInGoogle, signWithGoogle }
  }, [status, error, isLoggingIn, isLoggingInGoogle])
