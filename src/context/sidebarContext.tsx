...
export const SidebarProvider = ({ children }: { children: ReactNode | JSX.Element | JSX.Element[] }) => {
  ...
  const [mutateActive, setMutateActive] = useState<any>(null)
  ...
  function mutate(mutate: any) {
    setMutateActive(mutate)
  }
  ...
  const value = useMemo(() => ({
    ...
    mutate,
    mutateActive
  }), [isContract, selectedMenu, toggleContract, handleSelectedMenu, handleActivateMenu, menuActive, mutate])
